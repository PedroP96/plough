...
<body>
	<div class="mainContainer">
		...
		<div class="mainContent">
			<h1 class="pageTitle">Fruits and Vegetables</h1>
			<div class="columns">
				<div class="filtersColumn">
					<h2 id="filterTitle">Filters:</h2>
						<br>
						<a class="filters" href="fruits-and-veg.php?filter=fruits">Fruit</a>
						<br>
						<a class="filters" href="fruits-and-veg.php?filter=vegetables">Vegetables</a>
						<br>
						<a class="filters" href="fruits-and-veg.php?filter=all">Fruits &amp; Vegetables</a>
				</div>
				<div class="productsColumn">
					...
					if(!empty($_GET['sortBy'])) {
						if($_GET['sortBy'] == "nameAscending") {
							$query = "SELECT * FROM PRODUCTS WHERE P_TYPE='Fruit' OR P_TYPE = 'Vegetable' ORDER BY P_NAME DESC;";
						} elseif($_GET['sortBy'] == "nameDescending") {
							$query = "SELECT * FROM PRODUCTS WHERE P_TYPE='Fruit' OR P_TYPE = 'Vegetable' ORDER BY P_NAME ASC;";
						} elseif($_GET['sortBy'] == "priceAscending") {
							$query = "SELECT * FROM PRODUCTS WHERE P_TYPE='Fruit' OR P_TYPE = 'Vegetable' ORDER BY P_PRICE ASC;";
						} elseif($_GET['sortBy'] == "priceDescending") {
							$query = "SELECT * FROM PRODUCTS WHERE P_TYPE='Fruit' OR P_TYPE = 'Vegetable' ORDER BY P_PRICE DESC;";
						}
					}elseif(!empty($_GET['filter'])) {
						if($_GET['filter'] == "fruits") {
							$query = "SELECT * FROM PRODUCTS WHERE P_TYPE='Fruit';";
						} elseif($_GET['filter'] == "vegetables") {
							$query = "SELECT * FROM PRODUCTS WHERE P_TYPE='Vegetable';";
						} else {
							$query = "SELECT * FROM PRODUCTS WHERE P_TYPE='Fruit' OR P_TYPE = 'Vegetable';";
						}
					}else {
						$query = "SELECT * FROM PRODUCTS WHERE P_TYPE='Fruit' OR P_TYPE = 'Vegetable';";
					}
					...
					<div class="dropdown">
  					<button onclick="dropdownButton_js()" class="dropbtn">Sort By:</button>
  						<div id="myDropdown" class="dropdown-content">
							<a href="fruits-and-veg.php?sortBy=nameDescending">Name: A to Z</a>
                			<a href="fruits-and-veg.php?sortBy=nameAscending">Name: Z to A</a>
                			<a href="fruits-and-veg.php?sortBy=priceAscending">Price: Lowest to Highest</a>
                			<a href="fruits-and-veg.php?sortBy=priceDescending">Price: Hight to Lowest</a>
					  </div>
					</div>
				</div>
			</div>
